    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10 print:py-0 print:space-y-6 print:max-w-none">
      ...
      <div className="flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg glass-panel hover:bg-white/[0.10] text-xs text-[#CCCCCC] transition-colors cursor-pointer"
          >
            {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Share2 className="w-3.5 h-3.5" />}
            <span>{copied ? 'Link Copied' : 'Share'}</span>
          </button>
          <button
            id="print-instruction-sheet-btn"
            onClick={() => window.print()}
            title="Print or save as clean PDF instruction sheet"
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl glass-panel hover:bg-white/[0.12] border border-white/[0.15] hover:border-blue-400/40 text-xs text-white hover:text-blue-200 transition-all cursor-pointer shadow-sm group"
          >
            <Printer className="w-3.5 h-3.5 text-blue-400 group-hover:scale-110 transition-transform" />
            <span className="font-semibold">Print Sheet</span>
            <span className="hidden sm:inline-block text-[10px] px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-300 font-mono">PDF</span>
          </button>
        </div>
      </div>
      ...
      <div className="glass-card p-6 sm:p-8 rounded-2xl border border-white/10 space-y-4 print:p-4 print:space-y-2 print:border-gray-300">
        <div className="flex flex-wrap items-center gap-2 text-xs print:text-[9pt]">
          <span className="font-mono font-semibold px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 uppercase print:bg-gray-100 print:text-gray-900 print:border-gray-300">
            {workflow.category}
          </span>
        ...
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white tracking-tight print:text-gray-950 print:text-xl print:font-bold">
          {workflow.title}
        </h1>

        <p className="text-sm text-[#A3A3A3] leading-relaxed print:text-gray-800 print:text-[10pt]">
          {workflow.summary}
        </p>

        <div className="pt-2 flex items-center gap-2 text-xs text-[#666666] print:text-gray-600 print:pt-0 print:text-[9pt]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {workflow.devicesRequired.map((dev, idx) => (
            <div key={idx} className="p-3.5 rounded-xl glass-panel text-xs space-y-1">
              ...
              <div className="text-blue-400 font-mono text-[11px]">{dev.minOS}</div>
              {dev.hardwareNotes && (
                <div className="text-[#888888] text-[11px]">{dev.hardwareNotes}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="glass-card p-6 rounded-2xl border border-white/15 space-y-4">
        ...
        <div className="space-y-2">
          {workflow.requiredSettings.map((setting, sidx) => (
            <div key={sidx} className="flex items-start gap-2.5 p-3 rounded-xl glass-panel text-xs text-[#CCCCCC]">
      ...
      <div className="space-y-4 print:space-y-2">
        <div className="flex items-center justify-between border-b border-white/[0.08] pb-2 print:border-gray-300">
          <h2 className="text-xs font-semibold text-white uppercase tracking-wider print:text-gray-900 print:text-xs print:font-bold">
            Step-by-Step Execution Guide
          </h2>
          <span className="text-xs text-[#888888] print:text-gray-600 print:font-mono print:text-[9pt]">
                ...
                <div className="flex items-start justify-between gap-3 mb-3 print:mb-1">
                  <div className="flex items-center gap-3 print:gap-2">
                    ...
                    <button
                      ...
                      className={`no-print w-7 h-7 rounded-full text-xs font-bold flex items-center justify-center transition-all cursor-pointer ${
                        isCompleted
                    ...
                    <div className="hidden print:inline-flex items-center gap-1.5">
                      ...
                      <span className="font-bold text-xs font-mono text-gray-900">Step {step.stepNumber}:</span>
                    </div>
                    <h3 className={`text-base font-bold text-white print:text-gray-950 print:text-xs print:font-bold ${isCompleted ? 'line-through text-[#888888] print:no-underline' : ''}`}>
                      {step.title}
                    </h3>
                  </div>
                </div>

                <p className="text-xs text-[#A3A3A3] leading-relaxed pl-10 print:pl-5 print:text-gray-800 print:text-[9.5pt]">
                  {step.instruction}
                </p>
      ...
      {workflow.commonProblems && workflow.commonProblems.length > 0 && (
        <div className="glass-card p-6 rounded-2xl border border-white/15 space-y-4 print:p-4 print:border-gray-300 print:space-y-2">
          <h2 className="text-xs font-semibold text-white uppercase tracking-wider flex items-center gap-2 print:text-gray-900 print:font-bold">
            <AlertCircle className="w-4 h-4 text-amber-400 no-print" />
          ...
          <div className="space-y-3 print:space-y-1.5">
            {workflow.commonProblems.map((prob, pidx) => (
              <div key={pidx} className="p-4 rounded-xl glass-panel border-amber-500/30 text-xs space-y-1.5 print:p-2.5 print:border-gray-300 print:bg-gray-50 print:text-gray-900">
                <div className="font-semibold text-amber-300 print:text-amber-900">Symptom: {prob.issue}</div>
      ...
      <div className="glass-card p-6 rounded-2xl border border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.08)] space-y-3 print:p-4 print:border-gray-300 print:shadow-none">
        <h2 className="text-xs font-semibold text-white uppercase tracking-wider flex items-center gap-2 text-emerald-400 print:text-emerald-900 print:font-bold">
          <ShieldCheck className="w-4 h-4 no-print" />
